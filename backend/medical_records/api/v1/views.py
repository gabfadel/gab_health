from medical_records.api.v1.serializers import MedicalRecordSerializer, MedicalRecordCreateSerializer, MedicationSerializer
...
class MedicalRecordViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not hasattr(user, 'is_doctor') or not user.is_doctor:
            return Response({"detail": "You do not have permission to create a medical record."}, status=403)
        ...
        for med_name in medications:
            ...
            if created:
                ...
                if status_code == 200 and data:
                    ...
                    medication.brand_name = next(iter(extracted.get('brand_names', [])), '') or None
                    medication.generic_name = next(iter(extracted.get('generic_names', [])), '') or None
                    medication.manufacturer = next(iter(extracted.get('manufacturers', [])), '') or None
                    medication.dosage_form = next(iter(extracted.get('dosage_forms', [])), ' ') or None
                    medication.route = next(iter(extracted.get('routes', [])), '') or None
                    medication.substance_name = next(iter(extracted.get('substance_names', [])), '') or None
                    medication.pharm_class = next(iter(extracted.get('pharm_classes', [])), '') or None
                    
                    medication.known_reactions = ', '.join(extracted.get('reactions', [])) or None
    ...
    def update(self, request, *args, **kwargs):
        user = self.request.user
        if not hasattr(user, 'is_doctor') or not user.is_doctor:
            return Response({"detail": "You do not have permission to update a medical record."}, status=403)
        
        record = self.get_object()
        if record.doctor != user:
            return Response({"detail": "You do not have permission to update this medical record."}, status=403)
    ...
    def destroy(self, request, *args, **kwargs):
        user = self.request.user
        if not hasattr(user, 'is_doctor') or not user.is_doctor:
            return Response({"detail": "You do not have permission to delete a medical record."}, status=403)
        record = self.get_object()
        if record.doctor != user:
            return Response({"detail": "You do not have permission to delete this medical record."}, status=403)
    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            ...
            properties={
                'query': openapi.Schema(type=openapi.TYPE_STRING, description="Termo de busca para medicamento")
            },
            required=['query']
        ),
