from appointments.api.v1.serializers import AppointmentCreateSerializer, AppointmentDetailSerializer, AppointmentSerializer
...
class AppointmentViewSet(viewsets.ModelViewSet):
    ...
    def confirm(self, request, pk=None):
        ...
        if not (request.user.is_staff or appointment.doctor == request.user):
            return Response({"detail": "You do not have permission to confirm this appointment."}, status=403)
        if appointment.is_confirmed:
            return Response({"detail": "This appointment is already confirmed."}, status=400)
        if appointment.is_canceled:
            return Response({"detail": "This appointment has been canceled and cannot be confirmed."}, status=400)  
    ...
    def cancel(self, request, pk=None):
        appointment = self.get_object()
        if not (request.user.is_staff or appointment.doctor == request.user):
            return Response({"detail": "You do not have permission to cancel this appointment."}, status=403)
        if appointment.is_canceled:
            return Response({"detail": "This appointment is already canceled."}, status=400)
        if appointment.is_confirmed:
            return Response({"detail": "This appointment has been confirmed and cannot be canceled."}, status=400)  
